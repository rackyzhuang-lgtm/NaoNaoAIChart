...
- macOS job 不设置 `ELECTRON_BUILDER_BINARIES_MIRROR`，electron-builder 使用默认官方辅助二进制来源。
...
- macOS 失败已定位为 npmmirror 缺少 `dmgbuild-bundle-arm64-75c8a6c.tar.gz`，不是应用构建、ad-hoc 签名或 notarization 错误。
