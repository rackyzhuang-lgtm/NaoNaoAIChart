...
- macOS job 已移除 `ELECTRON_BUILDER_BINARIES_MIRROR`，仅对 electron-builder 辅助二进制恢复默认官方来源；Electron 本体与 npm 包继续使用现有 npmmirror。Windows 已通过的镜像配置保持不变。
...
- 本批远端 macOS 重跑和 Release 资产列表尚待 `v1.22.1` 标签运行确认，不得提前写成通过。
