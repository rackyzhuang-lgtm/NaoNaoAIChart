...
- macOS job 已显式设置优先级更高的 `NPM_CONFIG_ELECTRON_BUILDER_BINARIES_MIRROR` 到官方 GitHub Release 基地址，覆盖 runner 注入的 npmmirror 配置；Electron 本体与 npm 包继续使用现有 npmmirror。Windows 已通过的镜像配置保持不变。
...
- `v1.22.1` 首次标签运行仍因隐藏 npm 镜像优先级失败；修复后需移动该未发布标签并重新运行，远端 macOS 与 Release 资产列表尚待确认，不得提前写成通过。
