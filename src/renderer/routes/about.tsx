import { Anchor, Box, Button, Container, Flex, Image, Progress, Stack, Text, Title } from '@mantine/core'
import { IconChevronRight, IconHome, IconRefresh } from '@tabler/icons-react'
          ...
          <Flex gap="xxl" p="md" className="rounded-lg bg-chatbox-background-secondary">
            ...
            <Stack flex={1} gap="xxs">
              <Flex justify="space-between" align="center" wrap="wrap" gap={isSmallScreen ? 'xs' : 'sm'} rowGap="xs">
                <Title order={5} lh={1.5} lineClamp={1} title={`NaoNaoAI Chat v${version.version}`}>
                  NaoNaoAI Chat {/\d/.test(version.version) ? `(v${version.version})` : ''}
                </Title>
 ...
 */
function UpdateSection({ needCheckUpdate: _needCheckUpdate }: { needCheckUpdate: boolean }) {
    ...
    case 'error':
      return (
        <Stack gap={2} align="flex-end" className="flex-shrink-0">
          ...
          <Anchor size="xs" c="chatbox-tertiary" onClick={() => platform.openLink('https://naonaoai.shop/')}>
