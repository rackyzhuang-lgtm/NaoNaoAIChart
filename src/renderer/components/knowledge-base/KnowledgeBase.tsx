...
const KnowledgeBasePage: React.FC = () => {
  ...
  const [newProviderMode, setNewProviderMode] = useState<'custom'>('custom')
  ...
  function formatParserType(parserType?: DocumentParserType): string {
    switch (parserType) {
      case 'chatbox-ai':
        return t('Legacy cloud parser (unavailable)')
      case 'mineru':
        return 'MinerU'
      default:
        return t('Local')
    ...
    <Stack p="md" gap="xl">
      ...
      <Modal opened={showCreate} onClose={() => setShowCreate(false)} title={t('Create Knowledge Base')} centered>
        <Stack gap="md">
          ...
          <KnowledgeBaseProviderModeSelect value={newProviderMode} onChange={setNewProviderMode} />
                ...
                <Stack gap="md">
                  <Stack gap="0">
                    ...
                    <Group gap="xs" wrap="wrap" align="center">
                      {kb.providerMode === 'chatbox-ai' ? (
                        <>
                          <Text size="xs" c="dimmed">
                            {t('Models')}:
                          </Text>
                          <ModelPill
                            modelValue={t('Legacy provider')}
                            formatModelName={() => t('Legacy provider')}
                            isProviderAvailable={() => false}
                            type="embedding"
                            t={t}
                            unavailableTooltip={String(t('Legacy provider unavailable'))}
                          />
                        </>
                      ) : (
                        <>
